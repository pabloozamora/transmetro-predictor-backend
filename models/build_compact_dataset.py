...
cat_cols = ["LINEA","DIR","proxima_est_teorica","DIR_init","Motivo"]
num_cols = [
...
def _mask_valid(chunk: pd.DataFrame) -> pd.Series:
    m = (
        ...
        & (chunk.get("ETA_proxima_est_s") > 0)
        & chunk.get("dist_a_prox_m").between(0, 3000)
        & (
            (chunk.get("vel_mps") > 0.3)
            | (chunk.get("dwell_same_xy_s").fillna(0) < 240)
        )
    )
    # Si alguna columna no existe, get() devuelve None → provoca error; por eso aseguramos tipos antes.
    return m
